...
export function uploadFile<T>(path: string, filePath: string, name = 'file', formData: Record<string, string> = {}, onProgress?: (progress: number) => void, retry = true): Promise<T> {
  if (IS_MOCK) return Promise.reject(new ApiError('真实接口在 Mock 模式下不可调用'));
  return new Promise((resolve, reject) => { const task = uni.uploadFile({ url: `${API_BASE}${path}`, filePath, name, formData, header: readAccessToken() ? { Authorization: `Bearer ${readAccessToken()}` } : {}, success: (res) => { void (async () => { try { const parsed = typeof res.data === 'string' ? JSON.parse(res.data) : res.data; const body = parsed as ApiEnvelope<T>; if (res.statusCode === 401 && retry && await refreshAccessToken()) { resolve(await uploadFile<T>(path, filePath, name, formData, onProgress, false)); return; } if (res.statusCode === 401) clearTokens(); if (res.statusCode < 200 || res.statusCode >= 300) throw new ApiError(body.message ?? '上传失败', res.statusCode, body); resolve(envelopeData(body)); } catch (error) { reject(error); } })(); }, fail: (error) => reject(new ApiError((error as { errMsg?: string }).errMsg ?? '上传失败')) }); task.onProgressUpdate((event) => onProgress?.(event.progress)); });
}
export interface DownloadResult { statusCode: number; tempFilePath: string; }
export function downloadFile(path: string, retry = true): Promise<DownloadResult> { return new Promise((resolve, reject) => uni.downloadFile({ url: `${API_BASE}${path}`, header: readAccessToken() ? { Authorization: `Bearer ${readAccessToken()}` } : {}, success: (res) => { void (async () => { if (res.statusCode === 401 && retry && await refreshAccessToken()) { resolve(await downloadFile(path, false)); return; } if (res.statusCode === 401) clearTokens(); if (res.statusCode >= 200 && res.statusCode < 300) resolve(res as DownloadResult); else reject(new ApiError('下载失败', res.statusCode)); })().catch(reject); }, fail: (error) => reject(new ApiError((error as { errMsg?: string }).errMsg ?? '下载失败')) })); }
